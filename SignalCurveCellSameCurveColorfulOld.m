function SignalCurveCellSameCurveColorfulOld(Signal, idle, Tbit)
...
for i = 1:numberOfCurves
    ...
    Tbit = Signal(i).Tbit;
    [X(i).plot,Y(i).plot, X(i).indexMatrix] = XandYGeneratorSameCurveShowAndColorOld(Signal(i).channel, Nbits, idle, Tbit);
%     [X(i).plot,Y(i).plot, X(i).indexMatrix,X(i).colors,X(i).texts] = XandYGeneratorSameCurve(Signal(i).channel, Nbits, idle, Tbit, Signal(i).type);
end
